function calc_GH_dynamic_non_const2!(mesh::mesh_type, material::Vector{material_table_type}, solver_var::solver_var_type, frequency::Float64)
    ...
    Nc = calc_N_matrix(csis_cont, solver_var.csi,solver_var.csi)
    dNcdcsi = calc_dNdcsi_matrix(csis_cont, solver_var.csi,solver_var.csi)
    dNcdeta = calc_dNdeta_matrix(csis_cont,solver_var.csi,solver_var.csi)
    ...
    for fe in 1:nelem
        ...
        for se in 1:nelem
            for n = 1:nnel
                ...
                if fe != se
                    HELEM, GELEM = calc_nonsing_dynamic(source_node,gauss_points,Nd,normal,J, solver_var.omega, delta, zconsts)
                else
